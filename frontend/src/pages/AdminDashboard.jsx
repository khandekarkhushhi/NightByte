...
function AdminDashboard() {
  ...
  // ── Socket: listen for new orders in real time ──────────────────────────
  useEffect(() => {
  const interval = setInterval(() => {
    const socket = getSocket();
    console.log("Checking socket:", socket?.id, "connected:", socket?.connected);
    
    if (!socket) return;

    console.log("Socket found, attaching new_order listener");
    
    socket.on("new_order", (newOrder) => {
      console.log("new_order received:", newOrder);
      setOrders((prev) => [newOrder, ...prev]);
      toast.success(
        `🔔 New order #${newOrder._id?.slice(-6).toUpperCase()} — ₹${newOrder.totalCost}`,
        { duration: 6000 }
      );
    });
